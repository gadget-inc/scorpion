...
class Crawl::LighthouseCrawlerTest < ActiveSupport::TestCase
  test "it crawls a test shop" do
    @property = create(:harry_test_charlie_property)
    @crawler = crawler_for_property(@property)
  ...
  def crawler_for_property(property)
    Crawl::LighthouseCrawler.new(property, create(:assessment_production_group, property: property))
  end
end
